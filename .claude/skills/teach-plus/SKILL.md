...
| **knowledge-asset** | **唯一知识出口** | **explain/practice/review 完成后结构化沉淀**（v5 强制，取代旧 knowledge/ 目录） |
...
teach-plus/review（周复盘 / 阶段复盘 → 推进 consolidation / review_due / mastered）
    ↓
knowledge-asset（学习笔记结构化沉淀 → knowledge-note 模式）（v5 新增）
    ↓
execution_guard（完成检查：study_plan_ref / practice_log_ref / learning_state 更新 + knowledge_asset_ref）
```
...
- **沉淀知识**（v5 强制）：explain/practice/review 完成后，将结构化学习内容交给 `knowledge-asset`（knowledge-note 模式）沉淀到 `.claude/skills/knowledge-asset/knowledge/knowledge-notes/`
